...
public class JDBCExample1 {
	public static void main(String[] args) {
		 ...
		 * 
		 * - Java에서 DB에 연결(접근) 할 수 있게 해주는 Java API (Java에서 제공하는 코드)
		 * 	-> Java.sql 패키지에 존재함
		 * 
		 */
		
		// Java코드를 이용해 EMPLOYEE 테이블에서 사번, 이름, 부서코드, 직급코드, 급여, 입사일 조회 후 이클립스 콘솔에 출력
		...
		try {
			...
			// - DB 연결 정보와 JDBC 드라이버를 이용해서 원하는 DB와 연결할 수 있는 Connection 객체를 생성하는 객체
			...
			// Connection 객체가 잘 생성 되었는지 확인 == DB 연결 정보에 오타가 없는지 확인
			...
			// 연결된 DB에 SQL을 전달하고 결과를 반환 받을 Statement 객체를 생성함
